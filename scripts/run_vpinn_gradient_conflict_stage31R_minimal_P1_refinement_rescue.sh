...
SCRIPT="${ROOT}/vpinn_gradient_conflict_stage31R_minimal_P1_refinement_rescue.py"
...
STAGE30="${ROOT}/vpinn_gradient_conflict_stage30R_p1_nullspace_localization.py"
STAGE30_DIR="${ROOT}/vpinn_gradient_conflict_stage30R_p1_nullspace_localization"
...
for file in "${SCRIPT}" "${STAGE3}" "${STAGE29}" "${STAGE30}"; do
  if [[ ! -f "${file}" ]]; then
...
for dir in "${STAGE29_DIR}" "${STAGE30_DIR}"; do
  if [[ ! -d "${dir}" ]]; then
...
exec "${PYTHON}" "${SCRIPT}" \
  ...
  --stage30-script "${STAGE30}" \
  --stage30-dir "${STAGE30_DIR}"
