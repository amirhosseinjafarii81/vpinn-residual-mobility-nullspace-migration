...
SCRIPT="${ROOT}/vpinn_gradient_conflict_stage32R_nullspace_migration_audit.py"
...
for file in "${SCRIPT}" "${STAGE3}" "${STAGE29}" "${STAGE31}"; do
  if [[ ! -f "${file}" ]]; then
...
for dir in "${STAGE29_DIR}" "${STAGE31_DIR}"; do
  if [[ ! -d "${dir}" ]]; then
