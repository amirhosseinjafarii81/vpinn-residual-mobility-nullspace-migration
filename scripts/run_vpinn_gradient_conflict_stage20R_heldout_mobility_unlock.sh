...
SCRIPT="${ROOT}/vpinn_gradient_conflict_stage20R_heldout_mobility_unlock.py"
...
STAGE19_DIR="${ROOT}/vpinn_gradient_conflict_stage19R_temporal_conflict_parity"
...
for file in "${SCRIPT}" "${STAGE3}" "${STAGE18}" "${STAGE19}"; do
  if [[ ! -f "${file}" ]]; then
...
if [[ ! -d "${STAGE19_DIR}" ]]; then
  echo "ERROR: missing Stage-19 result directory: ${STAGE19_DIR}" >&2
  exit 2
fi
