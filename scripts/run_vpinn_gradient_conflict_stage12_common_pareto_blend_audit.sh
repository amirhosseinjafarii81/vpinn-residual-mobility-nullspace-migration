...
SCRIPT="${ROOT}/vpinn_gradient_conflict_stage12_common_pareto_blend_audit.py"
...
STAGE11_SCRIPT="${ROOT}/vpinn_gradient_conflict_stage11_joint_descent_cone_audit.py"
STAGE11_DIR="${ROOT}/vpinn_gradient_conflict_stage11_joint_descent_cone_audit"
...
for file in "${SCRIPT}" "${STAGE3}" "${STAGE9}" "${STAGE11_SCRIPT}"; do
  if [[ ! -f "${file}" ]]; then
...
for dir in "${STAGE5}" "${STAGE10}" "${STAGE11_DIR}"; do
  if [[ ! -d "${dir}" ]]; then
...
exec "${PYTHON}" "${SCRIPT}" \
  ...
  --stage10-dir "${STAGE10}" \
  --stage11-script "${STAGE11_SCRIPT}" \
  --stage11-dir "${STAGE11_DIR}"
